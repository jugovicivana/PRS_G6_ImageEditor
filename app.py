...
class ImageUploaderApp:
    ...
    def increase_saturation_parallel(self, num_parts):
        ...
        image_parts = self.split_image(num_parts)
        factor = 1.5  # Set your desired enhancement factor here

        start_time = time.time()  # Start timing

        # Process each part in parallel using concurrent.futures
    ...
    def split_image(self, num_parts):
        ...
        for i in range(num_parts):
            ...
            part = self.image_parallel.crop((0, top, width, bottom))
            image_parts.append(part)
